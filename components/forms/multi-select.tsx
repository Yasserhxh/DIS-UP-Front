    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          aria-expanded={open}
          className={cn(
            "w-full h-auto min-h-12 justify-between rounded-xl py-3",
        ...
        >
          <div className="flex flex-wrap gap-1">
            {selectedOptions.length > 0 ? (
              selectedOptions.map((option) => (
                <Badge key={option.value} variant="secondary" className={cn("mr-1 mb-1", badgeClassName)}>
                  {option.label}
                  <button
                    type="button"
          ...
          <div className="flex items-center gap-1">
            {selected.length > 0 && (
              <button type="button" className="rounded-full hover:bg-muted p-1" onClick={handleClear}>
